...
const APPOINTMENT_TYPES = [
  { value: 'prenatal_visit', label: 'Prenatal Visit', icon: 'fitness' },
  { value: 'birth_planning_session', label: 'Birth Planning', icon: 'document-text' },
  { value: 'postpartum_visit', label: 'Postpartum Visit', icon: 'heart' },
...
export default function MidwifeAppointmentsScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [appointmentsData, clientsData] = await Promise.all([
        ...
        apiRequest('/api/midwife/clients', { method: 'GET' }),
      ]);
  ...
  const handleCreateAppointment = async () => {
    ...
    try {
      ...
      await apiRequest('/api/appointments', {
        method: 'POST',
        body: {
          mom_user_id: selectedClient.linked_mom_id,
    ...
    Alert.alert(
      ...
      [
        { text: 'No', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await apiRequest(`/api/appointments/${appointmentId}`, { method: 'DELETE' });
  ...
  const getFilteredAppointments = () => {
    ...
    if (activeTab === 'prenatal') {
      ...
      filtered = appointments.filter(a => a.appointment_type === 'postpartum_visit');
    }
  ...
  const renderAppointmentCard = (appointment: Appointment) => (
    <Card key={appointment.appointment_id} style={styles.appointmentCard} data-testid={`appointment-${appointment.appointment_id}`}>
      <View style={styles.cardHeader}>
        <View>
          <Text style={styles.clientName}>{appointment.mom_name}</Text>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']} data-testid="midwife-appointments-screen">
      {/* Header */}
      <View style={styles.header}>
        ...
        >
          <Icon name="add-circle" size={28} color={clients.length > 0 ? COLORS.roleMidwife : COLORS.textLight} />
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.tabsContainer}>
        ...
        <TouchableOpacity
          style={[styles.tab, activeTab === 'prenatal' && styles.tabActive]}
        ...
        <TouchableOpacity
          style={[styles.tab, activeTab === 'postpartum' && styles.tabActive]}
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={isRefreshing} onRefresh={handleRefresh} colors={[COLORS.roleMidwife]} />
        }
      ...
      >
        {filteredAppointments.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyTitle}>No {activeTab !== 'all' ? activeTab : ''} Appointments</Text>
            ...
            <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                ...
                <View style={styles.pickerContainer}>
                  {clients.map(client => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.pickerItem,
                        selectedClient?.client_id === client.client_id && styles.pickerItemSelected,
                      ]}
                      onPress={() => {
                    ...
                    >
                      ...
                      {selectedClient?.client_id === client.client_id && (
                        <Icon name="checkmark" size={20} color={COLORS.roleMidwife} />
                      )}
                    </TouchableOpacity>
                  ))}
                </View>
              ...
              <View style={styles.typeGrid}>
                {APPOINTMENT_TYPES.map(type => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.typeCard,
                      appointmentType === type.value && styles.typeCardSelected,
                    ]}
                  ...
                  >
                    <Icon
                      ...
                      size={24}
                      color={appointmentType === type.value ? COLORS.roleMidwife : COLORS.textSecondary}
                    />
                    <Text style={[
                      styles.typeLabel,
                      appointmentType === type.value && styles.typeLabelSelected,
                    ]}>
                      {type.label}
                    </Text>
                  </TouchableOpacity>
              ...
              >
                <View style={styles.toggleLeft}>
                  <Icon name="videocam" size={20} color={COLORS.roleMidwife} />
                  <Text style={styles.toggleLabel}>Virtual Meeting</Text>
                </View>
                <View style={[styles.toggle, isVirtual && styles.toggleActive]}>
