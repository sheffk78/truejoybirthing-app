...
export default function SignMidwifeContractScreen() {
  ...
  const fetchContract = async () => {
    ...
    try {
      const data = await apiRequest<MidwifeContractData>(`${API_ENDPOINTS.MIDWIFE_CONTRACT_BY_ID}/${id}`);
  ...
  const handleSign = async () => {
    ...
    setSigning(true);
    try {
      await apiRequest(`${API_ENDPOINTS.MIDWIFE_CONTRACT_BY_ID}/${id}/sign`, {
        method: 'POST',
        body: {
          signer_name: signerName.trim(),
          signature_data: signerName.trim(), // Using typed name as signature
        },
      });
