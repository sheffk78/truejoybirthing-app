...
async def send_doula_contract(contract_id: str, user: User = Depends(check_role(["DOULA"]))):
    ...
    if client and client.get("linked_mom_id") and resend and _sender_email:
        ...
        if mom and mom.get("email"):
            try:
                signing_url = f"https://midwife-labor.preview.emergentagent.com/contract/{contract_id}"
                params = {
                    "from": _sender_email,
                    "to": mom["email"],
...
async def send_midwife_contract(contract_id: str, user: User = Depends(check_role(["MIDWIFE"]))):
    ...
    if client and client.get("linked_mom_id") and resend and _sender_email:
        ...
        if mom and mom.get("email"):
            try:
                signing_url = f"https://midwife-labor.preview.emergentagent.com/sign-midwife-contract?contractId={contract_id}"
                params = {
                    "from": _sender_email,
                    "to": mom["email"],
