...
class TestAuth:
    ...
    def test_login_midwife(self, api_client):
        ...
        data = response.json()
        midwife_token = data.get("session_token")
        assert midwife_token is not None
    ...
    def test_login_mom(self, api_client):
        ...
        data = response.json()
        mom_token = data.get("session_token")
        assert mom_token is not None
...
class TestMidwifeNotes:
    ...
    def test_get_midwife_notes_returns_array(self, api_client):
        ...
        global midwife_token
        assert midwife_token is not None, "Midwife not logged in"
...
class TestAdminPanel:
    ...
    def test_admin_endpoints_require_admin_role(self, api_client):
        ...
        global mom_token
        assert mom_token is not None, "Mom not logged in"
    ...
    def test_admin_users_endpoint(self, api_client):
        """GET /api/admin/users returns user list"""
        global admin_token
        
        # First, we need to create/get an admin user
        ...
        if reg_response.status_code == 200:
            admin_token = reg_response.json().get("session_token")
            print(f"Created new admin: {admin_email}")
        else:
            print(f"Admin registration failed: {reg_response.text}")
            # Try login in case admin exists
        ...
        if admin_token:
            ...
            pytest.skip("Could not create/login admin user")
    ...
    def test_admin_content_endpoint(self, api_client):
        ...
        if len(data) > 0:
            ...
            assert "explanatory_text" in item or item.get("explanatory_text") is None
        
        # Should have all birth plan sections
        assert len(data) >= 9, "Should have at least 9 birth plan sections"
