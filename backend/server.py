...
# /mom/midwife-visits - MIGRATED TO: routes/mom.py
...
async def send_contract(contract_id: str, user: User = Depends(check_role(["DOULA"]))):
    ...
    if client and client.get("linked_mom_id"):
        ...
        if mom and mom.get("email"):
            try:
                signing_url = f"https://backend-refactor-53.preview.emergentagent.com/contract/{contract_id}"
                params = {
                    "from": SENDER_EMAIL,
                    "to": mom["email"],
...
async def send_midwife_contract(contract_id: str, user: User = Depends(check_role(["MIDWIFE"]))):
    ...
    if client and client.get("linked_mom_id"):
        ...
        if mom and mom.get("email"):
            try:
                signing_url = f"https://backend-refactor-53.preview.emergentagent.com/sign-midwife-contract?contractId={contract_id}"
                params = {
                    "from": SENDER_EMAIL,
                    "to": mom["email"],
