...
async def send_contract(contract_id: str, user: User = Depends(check_role(["DOULA"]))):
    ...
    if client and client.get("linked_mom_id"):
        ...
        if mom and mom.get("email"):
            try:
                signing_url = f"https://joy-birthing-preview.preview.emergentagent.com/contract/{contract_id}"
                params = {
                    "from": SENDER_EMAIL,
                    "to": mom["email"],
...
async def send_midwife_contract(contract_id: str, user: User = Depends(check_role(["MIDWIFE"]))):
    ...
    if client and client.get("linked_mom_id"):
        ...
        if mom and mom.get("email"):
            try:
                signing_url = f"https://joy-birthing-preview.preview.emergentagent.com/sign-midwife-contract?contractId={contract_id}"
                params = {
                    "from": SENDER_EMAIL,
                    "to": mom["email"],
...
async def get_unified_appointments(
    ...
    for appt in appointments:
        ...
        if appt.get("client_id"):
            ...
            client = await db.clients.find_one({"linked_mom_id": appt["mom_user_id"], "pro_user_id": user.user_id}, {"_id": 0})
